...
import { ArrowLeft, ArrowRight, Star, Loader, AlertCircle, Search } from "lucide-react";
        ...
        <div className="flex flex-wrap gap-3 mb-8 items-center">
          <form onSubmit={applySearch} className="flex gap-2">
            <input
              ...
              className="px-4 py-2 rounded-lg bg-[#1f2937] border border-gray-700 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 w-56"
            />
            <button
              type="submit"
          ...
          {activeCategory && (
            <span className="flex items-center gap-2 px-3 py-1.5 bg-blue-900/40 border border-blue-600 rounded-full text-sm text-blue-300">
              {activeCategory}
              <button onClick={() => clearFilter("category")} className="hover:text-white">×</button>
            </span>
          )}
          {activeSearch && (
            <span className="flex items-center gap-2 px-3 py-1.5 bg-gray-700 rounded-full text-sm text-gray-300">
              "{activeSearch}"
              <button onClick={() => { clearFilter("search"); setSearchInput(""); }} className="hover:text-white">×</button>
            </span>
          )}
        </div>
            ...
            {courses.map((course) => (
              ...
              >
                ...
                <div className="p-6 flex flex-col flex-1">
                  ...
                  <p className="text-gray-300 text-sm mt-4 mb-6 flex-1 line-clamp-3">{course.description}</p>
